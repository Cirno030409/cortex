...
test_comment = f"Cortex - mc={params['n_mini_column']} labels={params['labels']} n={params['n_samples']}_speedtest" #! 実験用コメント
...
print(f"labels: {params['labels']}, n_samples: {params['n_samples']}, epoch: {params['epoch']}, n_mini_column: {params['n_mini_column']}\n")
...
for j in tqdm(range(params["epoch"]), desc="epoch progress", dynamic_ncols=True): # エポック数繰り返す
            ...
            if i < params["sync_weight_term"]: # 初期重み同期
                if j == 0:
                    for k in range(params["n_mini_column"]):
                        cortex.network[f"mc{k}_S_1"].w[:] = 0 # 重みを0に設定
                else:
                    for k in range(params["n_mini_column"]):
                        cortex.network[f"mc{k}_S_1"].w[:] = params_mc["static_synapse_params_ei"]["w"]
                        # model.network[f"mc{k}_N_1"].namespace["theta_dt"] = 0 # ホメスタシスの係数を0に設定
            cortex.run(params["exposure_time"])
            cortex.reset()
    ...
    # ===================================== ラベルの割り当て & 重みの保存 ==========================================
    for i in range(params["n_mini_column"]):
        weight = cortex.network[f"mc{i}_S_1"].w
        np.save(os.path.join(SAVE_PATH, f"mc{i}_weights.npy"), weight)
        assigned_labels = tools.assign_labels2neurons(cortex.network[f"mc{i}_spikemon_for_assign"], params_mc["n_e"], params["labels"], all_labels, params["exposure_time"], 0*ms) # ニューロンにラベルを割り当てる
    ...
    if SAVE_WEIGHT_CHANGE_GIF:
        for k in range(params["n_mini_column"]):
            plotter.weight_plot(cortex.network[f"mc{k}_S_0"], n_pre=params_mc["n_inp"], n_post=params_mc["n_e"], save_path=SAVE_PATH + f"LEARNING/", n_this_fig=f"weight_plot(mc{k})")
            tools.make_gif(25, SAVE_PATH + f"LEARNING/learning weight matrix/mini column{k}/", SAVE_PATH + "LEARNING/", f"weight_change(mc{k}).gif")
   
    tools.save_all_monitors(SAVE_PATH + f"LEARNING/Monitors/", cortex.network) # モニターを保存
