...
for weight in tqdm(exc_weights, desc="exc_weight progress", dynamic_ncols=True):
    ...
    test_comment = f"Cortex - mc={params['n_mini_column']} labels={params['learning_labels']} n={params['n_samples']}_初期重み同期_2epoch_sync200" #! 実験用コメント
    ...
    for j in tqdm(range(params["epoch"]), desc="epoch progress", dynamic_ncols=True): # エポック数繰り返す
        images, labels = Datasets.get_mnist_sample_equality_labels(params["n_samples"], labels=params["learning_labels"], dataset="train") # テスト用の画像とラベルを取得
